...
export default function PricingSection() {
  ...
  const sharedHostingCPanelPlans = [
    {
      ...
      domainType: ".com",
      controlPanel: "cPanel",
      features: ["2 Website", "10 GB SSD NVMe Storage ", "100 GB Bandwidth", "10 Email Accounts", "Basic Support", "Free SSL Let's Encrypt Certificate", "Free .com Domain"],
      highlighted: false,
    },
    {
      name: "Silver",
  ...
  const sharedHostingDAPlans = [
    {
      ...
      features: ["2 Website", "10 GB SSD NVMe Storage ", "100 GB Bandwidth", "10 Email Accounts", "Basic Support", "Free SSL Let's Encrypt Certificate", "Free .com or .org Domain"],
      highlighted: false,
    },
    {
      name: "Silver",
  ...
  const getResellerPlansWithPricing = () => {
    return resellerHostingPlans.map((plan) => {
      ...
      const domainLabel = plan.domainType === "(.*)" ? "1 Free Domain .com (Annual billing) (.*)" : `Free ${plan.domainType} Domain`
      return {
        name: plan.name,
        price: pricing.price,
                ...
                {plan.freeDomain && plan.domainType && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-medium text-slate-700 dark:text-slate-300">
                      {plan.domainType === "(.*)" ? "1 Free Domain .com (Annual billing) (.*)" : `Free ${plan.domainType} Domain`}
                    </span>
                  </div>
                )}
              </div>
          ...
          <p className="text-sm text-blue-900 dark:text-blue-200 flex items-start gap-2">
            ...
            <span>
              <strong>Tips:</strong> If you don't want to use our domain name, you can purchase a domain from another registrar and set the DNS to <code className="bg-white dark:bg-slate-800 px-2 py-1 rounded text-xs font-mono">ns1.kmerhosting.com</code> and <code className="bg-white dark:bg-slate-800 px-2 py-1 rounded text-xs font-mono">ns2.kmerhosting.com</code>
                <div className="mt-4">You can contact support to remove the domain name from your plan <strong>before purchasing</strong>, and the price will be recalculated.</div>
                <div className="mt-4">And if you want to add multiple domains to a specific KmerHosting service you have purchased, submit a request through your dashboard quick action by specifying the associated service.</div>
            </span>
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto mb-20">
            ...
            <div className="rounded-xl p-6 flex flex-col border-2 bg-white dark:bg-slate-900 transition-all duration-300 hover:shadow-lg" style={{ borderColor: "#128C7E" }}>
              ...
              <div className="space-y-2 mt-5 pt-5 border-t border-slate-200 dark:border-slate-700">
                {["Full Root Access", "1 IPv4 Address", "Linux or Windows", "Free SSL Let's Encrypt Certificate", "24/7 Support"].map((feature, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    <Check className="w-3 h-3 flex-shrink-0 mt-0.5" style={{ color: "#128C7E" }} />
                    <span className="text-xs text-slate-600 dark:text-slate-400">{feature}</span>
                  </div>
            ...
            <div className="relative rounded-xl p-6 flex flex-col border-2 bg-white dark:bg-slate-900 transition-all duration-300 hover:shadow-lg shadow-lg" style={{ borderColor: "#128C7E" }}>
              ...
              <div className="space-y-2 mt-5 pt-5 border-t border-slate-200 dark:border-slate-700">
                {["Full Root Access", "2 IPv4 Addresses", "Linux or Windows", "Free SSL Let's Encrypt Certificate", "Priority Support", "Weekly Backups"].map((feature, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    <Check className="w-3 h-3 flex-shrink-0 mt-0.5" style={{ color: "#128C7E" }} />
                    <span className="text-xs text-slate-600 dark:text-slate-400">{feature}</span>
                  </div>
            ...
            <div className="rounded-xl p-6 flex flex-col border-2 bg-white dark:bg-slate-900 transition-all duration-300 hover:shadow-lg" style={{ borderColor: "#128C7E" }}>
              ...
              <div className="space-y-2 mt-5 pt-5 border-t border-slate-200 dark:border-slate-700">
                {["Full Root Access", "4 IPv4 Addresses", "Linux or Windows", "Free SSL Let's Encrypt Certificate", "24/7 Priority Support", "Daily Backups"].map((feature, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    <Check className="w-3 h-3 flex-shrink-0 mt-0.5" style={{ color: "#128C7E" }} />
                    <span className="text-xs text-slate-600 dark:text-slate-400">{feature}</span>
                  </div>
